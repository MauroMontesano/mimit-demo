...
@Component({
  selector: 'eaf-old-input-date-time',
  templateUrl: './input-old-date-time.component.html',
  styleUrls: ['./input-old-date-time.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => InputOldDateTimeComponent),
      multi: true,
    },
    {
...
export class InputOldDateTimeComponent extends EngularInputDate {
  ...
  protected options: any = {
    ...
    onChangeDateTime: (value: unknown) => {
      this.onChange(value);
    },
    locale: 'it',
    useCurrent: false,
  ...
  setDateInDatePicker(date: DateModel) {
    ...
    if (date && this.initialized) {
      ...
      this.initialized.datetimepicker('setDate', date.toDate() || null);
      this.options['date'] = date || null;
      if (!date) {
        this.initialized.datetimepicker('destroy');
  ...
  initializeLibrary() {
    ...
    this.initialized = $(this.input.nativeElement).datetimepicker(this.options);
  }

  showDataPicker() {
    this.initialized.datetimepicker('show');
  }
  ...
  override setDisabledState(isDisabled: boolean): void {
    ...
    $(this.input.nativeElement).datetimepicker(isDisabled ? 'disable' : 'enable');
  ...
  public override onChange = ($event: any) => {
    ...
    this.value = new DateModel($event);

    if (this.onChangeCallBack && (!this.value || !old || this.value.toTimeString() !== old.toTimeString())) {
