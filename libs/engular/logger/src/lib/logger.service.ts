...
/**
 * Is the logger service of application. The goal of this logger service is replace console.log and the abuse of them.
 * With this service you can choose if activate logger only for few pieces of application or for all application. So when you open console everything became clear.
 */
export class LoggerService {
  constructor() {
    ...
    this.loggersWriters = {};
    this.isActive = localStorage.getItem('logger') === 'true'; //!environment.production;
  ...
  private infoStyle = 'border: 0px solid DarkBlue; color: DarkBlue ; padding:2px';
  ...
  private info(who: string, mainMessage: string, color: string, params: any[]) {
    let args = ['%cLOG   ' + who + ': ' + '%c' + mainMessage, 'color:' + color + ';font-weight:800', 'color:#666'];
  ...
  private debug(who: string, mainMessage: string, color: string, params: any[]) {
    let args = ['%cDEBUG   ' + who + ': ' + '%c' + mainMessage, 'color:' + color + ';font-weight:800', 'color:#666'];
    args = args.concat(params);
    this.oldConsole.warn.apply(this.oldConsole.log, args);
  }

  private error(who: string, mainMessage: string, color: string, params: any[]) {
    let args = ['%cERROR   ' + who + ': ' + '%c' + mainMessage, 'color:' + color + ';font-weight:800', 'color:#666'];
    args = args.concat(params);
  ...
  activeAll = () => {
    for (const key in this.loggersConfiguration) {
  ...
  toggleLogger = (active?: boolean) => {
    if (!this.consoleDisabled && active) {
      this.disableConsole();
    }
    this.isActive = !!active;
    for (const keyWriter in this.loggersWriters) {
      const writer = this.loggersWriters[keyWriter];
      this.injectWriter(writer, this.loggersConfiguration[writer.name]);
    }
    if (active) {
      this.oldConsole.log('%cLogger is active. To disable it use LoggerService.off()', this.infoStyle);
    } else {
      this.oldConsole.log('%cLogger is disabled. To active it use LoggerService.on()', this.infoStyle);
    }
  };
  help = () => {
    LoggerService._instance.oldConsole.log(
      "%c         `7MMF'                          \n			MM                            \n  ,6\"Yb.    MM         ,pW\"Wq.   .P\"Ybmmm \n 8)   MM    MM        6W'   `Wb :MI  I8   \n  ,pm9MM    MM      , 8M     M8  WmmmP\"   \n 8M   MM    MM     ,M YA.   ,A9 8M        \n `Moo9^Yo..JMMmmmmMMM  `Ybmd9'   YMMMMMb  \n								6'     dP \n								Ybmmmd'    . \n\n- list(): get all logger already available\n- active(name): active the logger with that name\n- disable(name): disable the logger with that name\n- on(): enable the logger service (default in dev)\n- off(): disable the logger service (default in prod)\n- autoActiveNewLogger(auto=true):active all new logger.\n- activeAll: active all logger available (not the new one)\n- disableAll: disable all active logger  \n ",
      'color:blue'
    );
  };
  /**
  ...
  injectWriter(writer: LoggerWriter, active: boolean) {
    ...
    if (active && this.isActive) {
      writer.info = (mainMessage: string, ...params: any[]) => {
        this.info(writer.name, mainMessage, writer.color, params);
      };
      writer.debug = (mainMessage: string, ...params: any[]) => {
        this.debug(writer.name, mainMessage, writer.color, params);
      };
      writer.error = (mainMessage: string, ...params: any[]) => {
        this.error(writer.name, mainMessage, writer.color, params);
      };
    } else {
